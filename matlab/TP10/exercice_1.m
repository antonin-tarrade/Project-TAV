...
% Affichage du sonagramme en décibels :
figure(Name='Transformée de  Fourier à court terme', Position=[0.4*L,0,0.6*L,0.6*H]);
valeurs_t = (0 : size(Yr, 2) - 1) * D / f_ech;	% Instant correspondant à chaque colonne
valeurs_f = (0 : size(Yr, 1) - 1) * f_ech / N;	% Fréquence correspondant à chaque ligne
S = 20 * log10(abs(Yr));				% Module au carré de la TFCT en décibels
imagesc(valeurs_t, valeurs_f, S);
axis xy;
set(gca, FontSize=20);
xlabel('Temps ($s$)', Interpreter='Latex', FontSize=30);
...
% Calcul de la transformée de Fourier inverse :
signal_restitue = ITFCT(Yr, f_ech, N, D, fenetre);
...
% Affichage du sonagramme en décibels :
figure(Name='Transformée de  Fourier à court terme', Position=[0.4*L,0,0.6*L,0.6*H]);
valeurs_t = (0 : size(Yi, 2) - 1) * D / f_ech;	% Instant correspondant à chaque colonne
valeurs_f = (0 : size(Yi, 1) - 1) * f_ech / N;	% Fréquence correspondant à chaque ligne
S = 20 * log10(abs(Yi));				% Module au carré de la TFCT en décibels
imagesc(valeurs_t, valeurs_f, S);
axis xy;
set(gca, FontSize=20);
xlabel('Temps ($s$)', Interpreter='Latex', FontSize=30);
...
% Calcul de la transformée de Fourier inverse :
signal_restitue = ITFCT(Yi, f_ech, N, D, fenetre);
